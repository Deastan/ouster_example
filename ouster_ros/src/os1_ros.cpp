...
sensor_msgs::Imu packet_to_imu_msg(const PacketMsg& p) {
    ...
    m.orientation.w = 0;
    // invert the axis
    m.linear_acceleration.x = imu_la_x(buf) * standard_g *-1.0;
    m.linear_acceleration.y = imu_la_y(buf) * standard_g *-1.0;
    m.linear_acceleration.z = imu_la_z(buf) * standard_g *-1.0;
    // invert the axis
    m.angular_velocity.x = imu_av_x(buf) * M_PI / 180.0 *-1.0;
    m.angular_velocity.y = imu_av_y(buf) * M_PI / 180.0 *-1.0;
    m.angular_velocity.z = imu_av_z(buf) * M_PI / 180.0 *-1.0;
