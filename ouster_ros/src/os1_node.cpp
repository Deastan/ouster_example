...
int main(int argc, char** argv) {
    ...
    if (replay_mode) {
        auto lidar_packet_sub = nh.subscribe<PacketMsg, const PacketMsg&>(
            "lidar_packets", 5, lidar_handler);//500
        auto imu_packet_sub = nh.subscribe<PacketMsg, const PacketMsg&>(
            "imu_packets", 5, imu_handler);//500
        ros::spin();
    } else {
        auto lidar_packet_pub = nh.advertise<PacketMsg>("lidar_packets", 5);//500
        auto imu_packet_pub = nh.advertise<PacketMsg>("imu_packets", 5);//500
